...
// Re-links an account to a subscription Stripe already knows about, for when local
// data got wiped (e.g. Render free-tier disk reset) but the person is still a real,
// paying subscriber. Reuses the existing license for that Stripe subscription if one
// was already minted, instead of creating a duplicate.
function linkRecoveredSubscription(email, stripeSubscriptionId) {
  const db = readDB();
  db.subscriptions = db.subscriptions || {};
  db.accounts = db.accounts || {};
  db.accounts[email] = db.accounts[email] || {};
  const existing = Object.entries(db.subscriptions).find(
    ([, rec]) => rec.stripeSubscriptionId === stripeSubscriptionId
  );
  const licenseKey = existing
    ? existing[0]
    : 'S-' + crypto.randomBytes(3).toString('hex').toUpperCase() + '-' + crypto.randomBytes(3).toString('hex').toUpperCase();
  db.subscriptions[licenseKey] = {
    stripeSubscriptionId,
    createdAt: existing ? existing[1].createdAt : Date.now()
  };
  db.accounts[email].subLicense = licenseKey;
  writeDB(db);
  return licenseKey;
